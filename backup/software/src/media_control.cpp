...
void start_listener(char type, unsigned int in_address[4]){
    ...
    //     if(type == 0){
    //         // video
    //         sprintf(start_string, "omxplayer -o hdmi -b --no-keys --timeout 0 http://%u.%u.%u.%u:8080/ms0.mkv?buffer_size 12000000B&", in_address[0], in_address[1], in_address[2], in_address[3]);
    //     }
    //     if(type == 1){
    //         // audio
    //         sprintf(start_string, "omxplayer -o hdmi -b --no-keys --timeout 0 http://%u.%u.%u.%u:8080/ms0.mkv?buffer_size 12000000B&", in_address[0], in_address[1], in_address[2], in_address[3]);
...
char update_movie(char stream_select, char input_option, char* input_src, char out_clients, char client){
    ...
    //     if(media_option == 6){
    //         #ifdef IS_SERVER
    //             send_media("fastforward", ms_ip);
    //         #endif
    //     }
    //     if(media_option == 7){
    //         #ifdef IS_SERVER
    //             send_media("rewind", ms_ip);
    //         #endif
    //     }
    //     if(media_option == 8){
    //         #ifdef IS_SERVER
    //             send_media("key key-jump+long", ms_ip);
    //         #endif
    //     }
    //     if(media_option == 9){
    //         #ifdef IS_SERVER
...
void send_media(const char input_string[MAX_INPUT_STRING], unsigned int address[4]){
